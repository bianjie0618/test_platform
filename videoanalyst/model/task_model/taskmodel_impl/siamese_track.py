...
class SiamTrack(ModuleBase):
    ...
    default_hyper_params = dict(pretrain_model_path="",
                                ...
                                trt_mode=False,             # tensor2rt是一个模型inference 加速优化包，这里我们不做研究，暂时不管
                                trt_fea_model_path="",
    ...
    def train_forward(self, training_data):
        ...
        # feature adjustment
        c_z_k = self.c_z_k(f_z)     # classification_input-z_kernel
        r_z_k = self.r_z_k(f_z)     # regression_input-z_kernel
        c_x = self.c_x(f_x)         # classification_imput-x
        r_x = self.r_x(f_x)         # regression_imput-x
        # feature matching
    ...
    def forward(self, *args, phase=None):
        ...
        fcos_score_final: torch.Tensor
            predicted score for bboxes, shape=(B, HW, 1)    # 综合度得分
        ...
        fcos_ctr_prob_final: torch.Tensor
            center-ness score, shape=(B, HW, 1)             # 中心度得分
        """
        if phase is None:
            phase = self._phase
        ...
        # used for template feature extraction (normal mode)
            ...
            if self._hyper_params["trt_mode"]:
                ...
                f_z = self.basemodel(target_img)
                # template as kernel
                c_z_k = self.c_z_k(f_z)     # 分类模板kernel：c_z_k
                r_z_k = self.r_z_k(f_z)     # 分类模板kernel：r_z_k
                # output
                out_list = [c_z_k, r_z_k, f_z]  # 返回值新增f_z
        elif phase == 'shared_feature':
            tracked_img, = args
            new_f_z = self.basemodel(tracked_img)
            return new_f_z
        ...
        # used for tracking one frame during test
            ...
            if len(args) == 3:
                search_img, c_z_k, r_z_k = args     # shape(c_z_k) = (256, 4, 4)
                if self._hyper_params["trt_mode"]:
                    c_x, r_x = self.trt_track_model(search_img)
                else:
                    # backbone feature
                    f_x = self.basemodel(search_img)    # shape(f_x) = (256, 28, 28)
                    # feature adjustment
                    c_x = self.c_x(f_x)      # shape(c_x) = (256, 26, 26)
                    r_x = self.r_x(f_x)
            elif len(args) == 4:
            ...
            # feature matching
            r_out = xcorr_depthwise(r_x, r_z_k)     # shape(r_out) = (256, 23, 23)
            c_out = xcorr_depthwise(c_x, c_z_k)
            # head
            fcos_cls_score_final, fcos_ctr_score_final, fcos_bbox_final, corr_fea = self.head(
    ...
    def _make_convs(self):
        ...
        self.c_x = conv_bn_relu(head_width, head_width, 1, 3, 0, has_relu=False)    # 经过此步操作，feature map的h、w各减小2
