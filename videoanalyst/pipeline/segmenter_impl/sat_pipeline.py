...
class StateAwareTracker(PipelineBase):
    ...
    default_hyper_params = dict(
        ...
        context_amount=0.5,
        mask_rect_lr=0.52,
        track_failed_score_th_for_locating=0.0,
        track_failed_score_th_for_seg_identify=0.0,
        lost_strategy_amplitude=0.6,
        cosine_sin_th=0.0,
    ...
    def init(self, im, state, benchmark_test = True, init_mask = None):
        # state: bbox in xywh format is given for initialization in case of tracking
        polygon_points = []
        def box_2_polygon_points(box: List):
            polygon_points.append([box[0], box[1]])
            polygon_points.append([box[0] + box[2], box[1]])
            polygon_points.append([box[0] + box[2], box[1] + box[3]])
            polygon_points.append([box[0], box[1] + box[3]])
        if benchmark_test:
            box_2_polygon_points(box=state)
            np_pts = np.array(polygon_points, dtype=np.int32)
            zero_mask = np.zeros((im.shape[0], im.shape[1]),
                                    dtype=np.uint8)
    ...
    def joint_segmentation(self, im_x, target_pos, target_sz, corr_feature,
        ...
        self._tracker.put_filtered_img(full_filtered_image)

        if self._tracker.get_track_score(                       # 我也需要在SiamFC上增益一个相似度得分接口！！！
        ) < self._hyper_params["track_failed_score_th_for_seg_identify"]:
    ...
    def lost_strategy(self):
        angle = (self._state['lost_times'] * 45) % 180
        new_context_amount = self._tracker.get_initial_context_amount() + round(math.sin(math.radians(angle)) * self._hyper_params['lost_strategy_amplitude'], 1)
        self._tracker.set_context_amount(new_context_amount)
        self._state['lost_times'] += 1
    
    def clear_lost_state(self):
        self._state['lost_times'] = 0
        self._tracker.reset_context_amount()
    ...
    def cropping_strategy(self, p_mask_b, track_pos=None, track_size=None):
        ...
        if conf_score > self._hyper_params['conf_score_thresh']:
            ...
            if len(contours) != 0 and np.max(cnt_area) > 10:
                ...
                seg_target_pos = mask_pos
                lr = self._hyper_params["mask_rect_lr"]
                seg_target_sz = self._state["state"][1] * (             # 貌似有多个mask 对应的矩形框；后确认，仅有一个矩形框
                    1 - lr) + mask_sz * lr
                # 计算分割结果的相似度得分
                cosine_sin = self._tracker.similarity_score(seg_target_pos, seg_target_sz)

                if self._state['state_score'] > self._hyper_params['state_score_thresh'] and cosine_sin > self._hyper_params['cosine_sin_th']:
                    new_target_pos = seg_target_pos
                    new_target_sz = seg_target_sz
                    self.clear_lost_state()
                    self._tracker.template_fusing()
                else:
                    if self._state["track_score"] > self._hyper_params[
                            "track_failed_score_th_for_locating"]:
                        new_target_pos, new_target_sz = track_pos, track_size
                        self.clear_lost_state()
                    else:
                        self.lost_strategy()
    ...
    def update(self, im):
        ...
        target_pos_track, target_sz_track, corr_feature = self._tracker.update(         # 显然要走一遍传统的SiamFc++跟踪网络，这样必然耗时
        ...
        if self._hyper_params['cropping_strategy']:
            target_pos, target_sz = self.cropping_strategy(
                pred_mask_b, target_pos_track, target_sz_track)     # 到这里其实已经将目标的bounding box计算出来了，到时候可以直接从这里返回目标的B-box
        else:
        ...
        # -------------------------数据集测试时候的返回形式，只有track_rect----------------------
        # 版本一
        # return self._mask_2_bbox(track_rect)
        # 版本二
        # return track_rect
        # ----------------------------------------------------------------------------------

    def _mask_2_bbox(self, track_rect):
        # return track_rect

        mask = (self._state['mask_in_full_image'] > 0.50).astype(np.uint8) * 2
        cnts, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL,
                                           cv2.CHAIN_APPROX_NONE)
        if len(cnts) == 0:
            return track_rect     # mask, track_rect
        boxes = np.zeros((len(cnts), 4))
        for i, cnt in enumerate(cnts):
            rect = cv2.boundingRect(cnt.reshape(-1, 2)) # x1,y1, w, h
            boxes[i] = rect
