...
def get_subwindow_tracking(im,
    ...
    im_patch = cv2.warpAffine(im,
                              ...
                              flags=(cv2.INTER_LINEAR | cv2.WARP_INVERSE_MAP),      # cv2.WARP_INVERSE_MAP 此项标志需要格外关注，inverse_map：缩小变成放大，正向移动变成负向移动, 并且是先移动，后缩放。
                              borderMode=cv2.BORDER_CONSTANT,
                              borderValue=tuple(map(int, avg_chans)))
    
    # cv2.imshow('Source image', im)
    # cv2.imshow('Cropped image', im_patch)
    # cv2.waitKey()
